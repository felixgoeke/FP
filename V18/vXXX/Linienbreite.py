...
untergrund["daten"] = untergrund["daten"] * (3634 / 78108)
...
plt.legend()
plt.tight_layout()
plt.savefig("./plots/Europium-Untergrund.pdf")
plt.clf()
...
grenzen = {
    "L": [10, 3, 6, 12, 5, 10, 6, 12, 12],
    "R": [10, 8, 3, 13, 6, 10, 10, 17, 15],
}
...
# Schreiben der Ergebnisse in eine txt Datei
with open("./build/peaks_results.txt", "w") as file:
    file.write("Energie\tIntensität\tUnsicherheit(I)\tN\tN_err\tPeak Position\n")
    for i, row in peaks.iterrows():
        file.write(
            f"{row['Energie']}\t{row['Intensität']}\t{row['Unsicherheit(I)']}\t{row['N']:.1f}\t{row['N_err']:.0f}\t{row['peaks']}\n"
        )